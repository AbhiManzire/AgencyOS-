...
import type { PaymentMethod } from '@/features/finance/payments/api/payment.types';
import { PAYMENT_METHOD_LABELS } from '@/features/finance/payments/api/payment.types';
import { useCreateInvoicePayment } from '@/features/finance/payments/hooks/use-create-payment';
...
export function RecordPaymentDrawer({
  ...
  const { mutateAsync: createPayment, isPending } = useCreateInvoicePayment(invoiceId);
  ...
  const handleSubmit = async (event: SyntheticEvent<HTMLFormElement>): Promise<void> => {
    ...
    if (values.reference.trim().length > 120) {
      setError('Reference must be 120 characters or fewer.');
      return;
    }

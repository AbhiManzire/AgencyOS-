...
import { useAssignLeadTag } from '@/features/sales/leads/tags/hooks/use-assign-lead-tag';
import { useLeadTags } from '@/features/sales/leads/tags/hooks/use-lead-tags';
import { useUnassignLeadTag } from '@/features/sales/leads/tags/hooks/use-unassign-lead-tag';
import { extractApiErrorMessage } from '@/lib/api/extract-api-error';
import { Can } from '@/lib/rbac';

interface LeadTagsPanelProps {
  readonly leadId: string;
  readonly readOnly?: boolean;
}

export function LeadTagsPanel({ leadId, readOnly = false }: LeadTagsPanelProps) {
  const { showToast } = useToast();
  const [tagName, setTagName] = useState('');
  const { data: tags = [], isLoading, error, refetch } = useLeadTags(leadId);
  const { mutateAsync: assignTag, isPending: isAssigning } = useAssignLeadTag(leadId);
  const { mutateAsync: unassignTag, isPending: isUnassigning } = useUnassignLeadTag(leadId);
  ...
  return (
    <div className="space-y-4">
      {tags.length === 0 ? (
        <EmptyState title="No tags yet" description="Add tags to segment this lead." />
      ) : (

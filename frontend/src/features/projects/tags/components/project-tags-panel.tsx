...
import {
  useAssignProjectTag,
  useProjectTags,
  useUnassignProjectTag,
} from '@/features/projects/tags/hooks/use-project-tags';
...
export function ProjectTagsPanel({ projectId, readOnly = false }: ProjectTagsPanelProps) {
  const { showToast } = useToast();
  const [tagName, setTagName] = useState('');
  const { data: tags = [], isLoading, error, refetch } = useProjectTags(projectId);
  const { mutateAsync: assignTag, isPending: isAssigning } = useAssignProjectTag(projectId);
  const { mutateAsync: unassignTag, isPending: isUnassigning } = useUnassignProjectTag(projectId);
  ...
  return (
    <div className="space-y-4">
      {tags.length === 0 ? (
        <EmptyState title="No tags yet" description="Add tags to segment this project." />
      ) : (
        <div className="flex flex-wrap gap-2">
          {tags.map((tag) => (
            <StatusBadge key={tag.id} variant="neutral" className="gap-1 pr-1">
              {tag.name}
              {readOnly ? null : (
                <Can permission="projects.update" mode="hide">
                  <button
                    type="button"

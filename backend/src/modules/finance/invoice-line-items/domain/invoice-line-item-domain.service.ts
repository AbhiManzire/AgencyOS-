import type { InvoiceRecord } from '../../invoices/repositories/invoice.repository.interface';
import {
  INVOICE_LINE_ITEM_DOMAIN_ERROR_CODES,
  InvoiceLineItemDomainError,
} from './invoice-line-item-domain.errors';
import type {
  CreateInvoiceLineItemValidationInput,
  UpdateInvoiceLineItemValidationInput,
} from './invoice-line-item-domain.types';

export class InvoiceLineItemDomainService {
  validateCreate(input: CreateInvoiceLineItemValidationInput): void {
  ...
  validateUpdate(
    ...
    input: UpdateInvoiceLineItemValidationInput,
  ): void {
  ...
  assertInvoiceIsActive(invoice: InvoiceRecord): void {
    if (invoice.deletedAt !== null) {
      throw new InvoiceLineItemDomainError(
        INVOICE_LINE_ITEM_DOMAIN_ERROR_CODES.INVOICE_ARCHIVED,
  ...
  private assertNameRequired(name: string): void {
    if (name.trim().length === 0) {
      throw new InvoiceLineItemDomainError(
        INVOICE_LINE_ITEM_DOMAIN_ERROR_CODES.NAME_REQUIRED,
  ...
  private assertQuantityValid(quantity: number): void {
    if (!Number.isFinite(quantity) || quantity <= 0) {
      throw new InvoiceLineItemDomainError(
        INVOICE_LINE_ITEM_DOMAIN_ERROR_CODES.INVALID_QUANTITY,
  ...
  private assertUnitPriceValid(unitPrice: number): void {
    if (!Number.isFinite(unitPrice) || unitPrice < 0) {
      throw new InvoiceLineItemDomainError(
        INVOICE_LINE_ITEM_DOMAIN_ERROR_CODES.INVALID_UNIT_PRICE,
  ...
  private assertDiscountValid(discount: number): void {
    if (!Number.isFinite(discount) || discount < 0) {
      throw new InvoiceLineItemDomainError(
        INVOICE_LINE_ITEM_DOMAIN_ERROR_CODES.INVALID_DISCOUNT,
  ...
  private assertTaxValid(tax: number): void {
    if (!Number.isFinite(tax) || tax < 0) {
      throw new InvoiceLineItemDomainError(
        INVOICE_LINE_ITEM_DOMAIN_ERROR_CODES.INVALID_TAX,
  ...
  private assertSortOrderValid(sortOrder: number): void {
    if (!Number.isInteger(sortOrder) || sortOrder < 0) {
      throw new InvoiceLineItemDomainError(
        INVOICE_LINE_ITEM_DOMAIN_ERROR_CODES.INVALID_SORT_ORDER,
  ...
  private assertLineItemIsActive(lineItem: { deletedAt: Date | null }): void {
    if (lineItem.deletedAt !== null) {
      throw new InvoiceLineItemDomainError(
        INVOICE_LINE_ITEM_DOMAIN_ERROR_CODES.LINE_ITEM_ARCHIVED,
